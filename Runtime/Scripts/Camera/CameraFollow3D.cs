...
namespace DredPack.Camera
{
    public class CameraFollow3D : SimpleSingleton<CameraFollow3D>
    {
        ...
        public void LT_UpdatePosition(float deltaTime)
        {
            ...
            if (LT_Rotating)
                transform.rotation =
                    Quaternion.Lerp(transform.rotation, Target.rotation, LT_RotateSpeed * deltaTime / 2f);
        }
        ...
        public void UpdateLT(float deltaTime)
        {
            ...
            if (distanceToTarget > FT_MaxDistanceToTarget)
            {
                transform.position += new Vector3(direction.x * FT_MoveAxis.x,
                                          (FT_LockY && UpType == UpTypes.World) ? 0f : direction.y * FT_MoveAxis.y,
            ...
            public override void OnInspectorGUI()
            {
                ...
                switch (T.FollowType)
                {
                    ...
                    case FollowTypes.FollowForTarget:
                        ...
                        T.FT_MaxDistanceToTarget =
                            EditorGUILayout.FloatField("Max Distance To Target", T.FT_MaxDistanceToTarget);
                        EditorGUILayout.Space();
